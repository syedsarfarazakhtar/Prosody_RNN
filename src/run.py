...
for grid_search in params:
    ...
    for sample in samples:
        ...
        evaluation_set = []
        testing_set = []
        mymodel = model.Model(input_size=len(cols[ind]), output_size=2, hidden_dim=hidden_dim, n_layers=n_layers)
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if epoch % 1 == 0:
                ...
                tmp = (hidden_dim, n_layers, lr, epoch)
                results_test[tmp] = {}
                results_test[tmp]["Accuracy"] = accuracy
                results_test[tmp]["Precision"] = precision
                results_test[tmp]["Recall"] = recall
                results_test[tmp]["F1"] = f1
                results_test[tmp]["Features"] = current_features_str
