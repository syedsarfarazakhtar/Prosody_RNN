...
def process_data(df, featcats):
    ...
    df['break_index'] = df['break_index'].fillna(value=0)
    y_labels = []
    indices = []
    for index, row in df.iterrows():
        if row['break_index'] == 0:
            y_labels.append('break' if row['autobi_break_index'] in [4, "4-", "4p", "4"] else 'nobreak')
        else:
            y_labels.append('break' if row['break_index'] in [4, "4-", "4p", "4"] else 'nobreak')
...
def test(clf, X_train, y_train, X_test, y_test, columns):
    ...
    print("accuracy=%.3f (random=%.3f) f1=%.3f precision=%.3f recall=%.3f" % (
        ...
        sklearn.metrics.f1_score(y_test, y_pred, pos_label="break"),
        sklearn.metrics.precision_score(y_test, y_pred, pos_label="break"),
        sklearn.metrics.recall_score(y_test, y_pred, pos_label="break")))
    ...
    print("-----------------")
    return sklearn.metrics.f1_score(y_test, y_pred, pos_label="break")
